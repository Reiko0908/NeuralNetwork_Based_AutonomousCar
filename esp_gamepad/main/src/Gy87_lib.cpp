...
void Gy87::config(
  ...
  if(this->acc_enable or this->gyro_enable){
    sensor_write_reg_single(I2C_MPU6050_ADDR, ACC_GYRO_PWR_MGMT_1, 0x00);
    sensor_write_reg_single(I2C_MPU6050_ADDR, DLPF_REG, dlpf_config);
  }

  if(this->acc_enable){
    sensor_write_reg_single(I2C_MPU6050_ADDR, ACC_AFS_SEL_REG, acc_afs_sel);
    switch(acc_afs_sel){
      case ACC_AFS_SEL_2G:
  ...
  if(this->gyro_enable){
    sensor_write_reg_single(I2C_MPU6050_ADDR, GYRO_FS_SEL_REG, gyro_fs_sel);
    switch(gyro_fs_sel){
  ...
  if(this->mag_enable){
    ...
    sensor_write_reg_single(I2C_MPU6050_ADDR, ACC_GYRO_INT_PIN_CFG_REG, 0b10);
    sensor_write_reg_single(I2C_MPU6050_ADDR, ACC_GYRO_USER_CONTROL_REG, 0x0);

    sensor_write_reg_single(
        I2C_HMC5883L_ADDR,
        MAG_CONFIG_REG_A,
...
void Gy87::update_gyro_reading(){
  ...
  sensor_read_reg_multi(I2C_MPU6050_ADDR, GYRO_FIRST_READ_REG, buf, ACC_GYRO_MAG_DATA_OUTPUT_READ_LEN);
  this->gyro.x = (int16_t)buf[0] << 8 | buf[1];
  this->gyro.y = (int16_t)buf[2] << 8 | buf[3];
  this->gyro.z = (int16_t)buf[4] << 8 | buf[5];
...
void Gy87::update_acc_reading(){
  uint8_t buf[ACC_GYRO_MAG_DATA_OUTPUT_READ_LEN];
  sensor_read_reg_multi(I2C_MPU6050_ADDR, ACC_FIRST_READ_REG, buf, ACC_GYRO_MAG_DATA_OUTPUT_READ_LEN);
  this->acc.x = (int16_t)buf[0] << 8 | buf[1];
  this->acc.y = (int16_t)buf[2] << 8 | buf[3];
  this->acc.z = (int16_t)buf[4] << 8 | buf[5];
...
void Gy87::update_mag_reading(){
  uint8_t buf[ACC_GYRO_MAG_DATA_OUTPUT_READ_LEN];
  sensor_read_reg_multi(I2C_HMC5883L_ADDR, MAG_FIRST_READ_REG, buf, ACC_GYRO_MAG_DATA_OUTPUT_READ_LEN);
  this->mag.x = (int16_t)buf[0] << 8 | buf[1];
  this->mag.z = (int16_t)buf[2] << 8 | buf[3];
  this->mag.y = (int16_t)buf[4] << 8 | buf[5];
...
void Gy87::update_gyro_z_reading(){
  uint8_t buf[GYRO_DATA_OUTPUT_Z_AXIS_READ_LEN];
  sensor_read_reg_multi(I2C_MPU6050_ADDR, GYRO_DATA_OUTPUT_Z_AXIS_REG, buf, GYRO_DATA_OUTPUT_Z_AXIS_READ_LEN);
  this->gyro.z = ((int16_t)buf[0] << 8 | buf[1]) / this->gyro_division_factor;
}
//-----------------------------------------------------------------------
...
void Gy87::calib_mag(){
  ...
  while(micros() - start_us < MAG_CALIB_DURATION_US){
