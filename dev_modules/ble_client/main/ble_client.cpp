...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param){
  ...
  switch (event){
    ...
    case ESP_GATTC_REG_EVT:{
      ...
      if(scan_ret) ESP_LOGE(GATTC_TAG, "set scan params error, error code = %x", scan_ret);
      break;
    }
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT:{
      ...
      if(ret_status != ESP_GATT_OK) ESP_LOGE(GATTC_TAG, "esp_ble_gattc_write_char_descr error");
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:{
      ...
      for (int i = 0; i < sizeof(write_char_data); ++i) write_char_data[i] = i % 256;
    ...
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
      ...
      switch (scan_result->scan_rst.search_evt){
        case ESP_GAP_SEARCH_INQ_RES_EVT:
          ...
          if(adv_name == NULL) break;
          if(strlen(server_name) != adv_name_len || strncmp((char *)adv_name, server_name, adv_name_len) != 0) break;
          ESP_LOGI(GATTC_TAG, "Device found %s", server_name);
          if(connect) break;
          connect = true;
...
extern "C" void app_main(void){
  ...
  ret = esp_bt_controller_init(&bt_cfg);                if(ret){ ESP_LOGE(GATTC_TAG, "%s initialize controller failed: %s", __func__, esp_err_to_name(ret)); return; }
  ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);      if(ret){ ESP_LOGE(GATTC_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret)); return; }
  ret = esp_bluedroid_init();                           if(ret){ ESP_LOGE(GATTC_TAG, "%s init bluetooth failed: %s", __func__, esp_err_to_name(ret)); return; }
  ret = esp_bluedroid_enable();                         if(ret){ ESP_LOGE(GATTC_TAG, "%s enable bluetooth failed: %s", __func__, esp_err_to_name(ret)); return; }
  ret = esp_ble_gap_register_callback(esp_gap_cb);      if(ret){ ESP_LOGE(GATTC_TAG, "%s gap register failed, error code = %x", __func__, ret); return; }
  ret = esp_ble_gattc_register_callback(esp_gattc_cb);  if(ret){ ESP_LOGE(GATTC_TAG, "%s gattc register failed, error code = %x", __func__, ret); return; }
  ret = esp_ble_gattc_app_register(PROFILE_A_APP_ID);   if(ret) ESP_LOGE(GATTC_TAG, "%s gattc app register failed, error code = %x", __func__, ret);
  ret = esp_ble_gatt_set_local_mtu(500);                if(ret) ESP_LOGE(GATTC_TAG, "set local  MTU failed, error code = %x", ret);
}
