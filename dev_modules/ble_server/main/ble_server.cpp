...
static char server_name[ESP_BLE_ADV_NAME_LEN_MAX] = "ESP_GATTS_DEMO";
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param){
  switch (event){
    ...
    case ESP_GATTS_REG_EVT:{
      ...
      esp_err_t ret = esp_ble_gap_set_device_name(server_name); if(ret) ESP_LOGE(GATTS_TAG, "set device name failed, error code = %x", ret);
#ifdef CONFIG_EXAMPLE_SET_RAW_ADV_DATA
      ret = esp_ble_gap_config_adv_data_raw(raw_adv_data, sizeof(raw_adv_data)); if(ret) ESP_LOGE(GATTS_TAG, "config raw adv data failed, error code = %x ", ret);
      adv_config_done |= adv_config_flag;
      ret = esp_ble_gap_config_scan_rsp_data_raw(raw_scan_rsp_data, sizeof(raw_scan_rsp_data)); if(ret) ESP_LOGE(GATTS_TAG, "config raw scan rsp data failed, error code = %x", ret);
      adv_config_done |= scan_rsp_config_flag;
#else
    //config adv data
      ret = esp_ble_gap_config_adv_data(&adv_data); if(ret) ESP_LOGE(GATTS_TAG, "config adv data failed, error code = %x", ret);
      adv_config_done |= adv_config_flag;
      //config scan response data
      ret = esp_ble_gap_config_adv_data(&scan_rsp_data); if(ret) ESP_LOGE(GATTS_TAG, "config scan response data failed, error code = %x", ret);
      adv_config_done |= scan_rsp_config_flag;
#endif
    ...
    case ESP_GATTS_WRITE_EVT: {
      ...
      if(!param->write.is_prep){
        ...
        if(gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
          ...
          else if(descr_value == 0x0000) ESP_LOGI(GATTS_TAG, "Notification/Indication disable");
          else{
    ...
    case ESP_GATTS_CREATE_EVT:{
      ...
      if(add_char_ret) ESP_LOGE(GATTS_TAG, "add char failed, error code =%x",add_char_ret);
      break;
    }
    ...
    case ESP_GATTS_ADD_CHAR_EVT: {
      ...
      if(get_attr_ret == ESP_FAIL) ESP_LOGE(GATTS_TAG, "ILLEGAL HANDLE");

      ESP_LOGI(GATTS_TAG, "the gatts demo char length = %x", length);
      for(int i = 0; i < length; i++) ESP_LOGI(GATTS_TAG, "prf_char[%x] =%x",i,prf_char[i]);
      esp_err_t add_descr_ret = esp_ble_gatts_add_char_descr(gl_profile_tab[PROFILE_A_APP_ID].service_handle, &gl_profile_tab[PROFILE_A_APP_ID].descr_uuid,
                                                             ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, NULL, NULL);
      if(add_descr_ret) ESP_LOGE(GATTS_TAG, "add char descr failed, error code =%x", add_descr_ret);
      break;
    }
  ...
  switch (event){
    ...
    case ESP_GATTS_WRITE_EVT: {
      ...
      if(param->write.is_prep)
        ...
        ESP_LOGI(GATTS_TAG, "value len %d, value ", param->write.len);
        ESP_LOG_BUFFER_HEX(GATTS_TAG, param->write.value, param->write.len);
        if(gl_profile_tab[PROFILE_B_APP_ID].descr_handle != param->write.handle || param->write.len != 2) break;
        uint16_t descr_value= param->write.value[1]<<8 | param->write.value[0];
        if(descr_value == 0x0001){
    ...
    case ESP_GATTS_DISCONNECT_EVT: break;
...
extern "C" void app_main(void){
  ...
  // esp_bt_controller_init(&bt_cfg);                        if(ret){ ESP_LOGE(GATTS_TAG, "%s initialize controller failed: %s", __func__, esp_err_to_name(ret)); return; }
  // esp_bt_controller_enable(ESP_BT_MODE_BLE);              if(ret){ ESP_LOGE(GATTS_TAG, "%s enable controller failed: %s", __func__, esp_err_to_name(ret)); return; }
  // esp_bluedroid_init();                                   if(ret){ ESP_LOGE(GATTS_TAG, "%s init bluetooth failed: %s", __func__, esp_err_to_name(ret)); return; }
  // esp_bluedroid_enable();                                 if(ret){ ESP_LOGE(GATTS_TAG, "%s enable bluetooth failed: %s", __func__, esp_err_to_name(ret)); return; }
  // esp_ble_gatts_register_callback(gatts_event_handler);   if(ret){ ESP_LOGE(GATTS_TAG, "gatts register error, error code = %x", ret); return; }
  // esp_ble_gap_register_callback(gap_event_handler);       if(ret){ ESP_LOGE(GATTS_TAG, "gap register error, error code = %x", ret); return; }
  // esp_ble_gatts_app_register(PROFILE_A_APP_ID);           if(ret){ ESP_LOGE(GATTS_TAG, "gatts app register error, error code = %x", ret); return; }
  // esp_ble_gatts_app_register(PROFILE_B_APP_ID);           if(ret){ ESP_LOGE(GATTS_TAG, "gatts app register error, error code = %x", ret); return; }
  // esp_ble_gatt_set_local_mtu(500);                        if(ret) ESP_LOGE(GATTS_TAG, "set local  MTU failed, error code = %x", ret);
